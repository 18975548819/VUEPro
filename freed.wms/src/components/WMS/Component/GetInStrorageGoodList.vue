...
<script>
import {requestInStroageList} from '../../../api/api'
...
export default {
    ...
    methods:{
        getInGoodListFun(){
            let self = this;
            self.loading = true;
            requestInStroageList({PageIndex:1,PageSize:100,MaterieId:self.obj.MaterieId,RepertoryId:self.obj.RepertoryId}).then(function(res){
                if(!res.hasErr)
                {
